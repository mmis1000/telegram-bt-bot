...
function downloadWithMessageId(torrent, bot, chatId, messageId) {
    ...
    status = status || storage.getItemSync(infoHash)
    
    return doDownload(torrent, bot, {chatId: chatId, messageId: messageId}, status)
}
...
function downloadWithInlineMessageId(torrent, bot, inlineMessageId) {
    ...
    return doDownload(torrent, bot, {inlineMessageId: inlineMessageId}, status)
}
