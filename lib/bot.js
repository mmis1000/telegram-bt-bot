...
function downloadAndTrack(torrent, chatId, reply_to_message_id) {
    ...
    .then(function (message) {
        ...
        return downlaod(torrent, donwloadFolderPath)
...
📥: ${formatNumber(status.downloadedSize)}/${formatNumber(status.totalSize)}`, 
            {
                chat_id: chatId,
...
`📁: <code>${escapeHtml(status.name)}</code>
📦: download finished!
🔗: <a href="${escapeHtml(folderUrl)}">Click to open folder</a>`, 
        {
            chat_id: chatId,
            message_id: messageId,
            reply_to_message_id,
