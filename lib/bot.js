...
bot.on('inline_query', function (query) {
    ...
    if (query.query.length <= 250) {
        bot.answerInlineQuery(inlineQueryId, [{
            type: 'article', 
            id: 'download-' + Math.random().toString(16).slice(2),
            title: 'download',
        ...
        bot.answerInlineQuery(inlineQueryId, [{
            type: 'article', 
            id: 'error-link-too-long',
            title: 'Link is too long',
            input_message_content: {
                message_text: 'Your magnet link is too long, and will be cut in inline command. Please use the /bt command with me in chat instead.'
            }
        }])
    }
})
...
bot.on('chosen_inline_result', function(result) {
    ...
    .then(function () {
        if (result.result_id.match(/^download\-/)) {
